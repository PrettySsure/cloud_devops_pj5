...
export async function searchBooks(idToken: string, params: string ): Promise<Book[]> {
  console.log('Fetching books')
  const editedParam = params.replace(/\s/g,'')

  const response = await Axios.get(`${apiEndpoint}/books/search/${editedParam}`, {
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${idToken}`
    },
